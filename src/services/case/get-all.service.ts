...
import { ICasesPagedData, ECaseStatus } from "@/types/case.type";
...
export async function GetAllCasesService(params: IGetAllCasesParams = {}): Promise<IResponse<ICasesPagedData>> {
  try {
    const { data } = await baseApi.get<IResponse<ICasesPagedData>>("/cases", { params });
    return data;
  } catch (error) {
...
export function useGetAllCases(params: IGetAllCasesParams = {}, config?: TQueryConfig<typeof GetAllCasesService>) {
